...
export default function CheckoutPage() {
  ...
  // Track when component has mounted to avoid SSR issues
  useEffect(() => {
    setMounted(true)
  }, [])

  // Redirect if cart is empty after component has mounted and cart has had time to load
  useEffect(() => {
    if (mounted && items.length === 0) {
      const timer = setTimeout(() => {
        router.push('/cart')
      }, 2000) // Give cart time to load from localStorage
  ...
  // Show loading while authentication is being checked or component is mounting
